...
| 狀態 | select | 學員狀態 |
| 關聯學員 | relation → 學員 DB | 共課學員（如個人 ↔ 共課帳號） |

> 購買時數和已上時數**不存在於學員 DB**，而是從繳費紀錄和打卡紀錄透過 FIFO 模型即時計算。
>
> **收費方式**：`套時數`（預先購買時數包，FIFO 消耗）或 `單堂`（每次上課單獨計費，逐堂繳費）。
>
> **關聯學員**：支援共課場景。例如 A 和 B 有時個別上課、有時以「AB」名義共課，透過關聯學員欄位連結三筆記錄，學員查詢時會同時顯示個人與共課紀錄，共課日期旁標示「(共)」。共課帳號（如「AB」）沒有 LINE ID，打卡時系統會自動通知各關聯的個人帳號。
>
...
| `上課注意事項` | 顯示 6 條課程規範（請假、退費、時長、熱身、安全、責任）；Flex 卡片設計，首次綁定 LINE 時自動顯示 |
| `選單` | 顯示功能選單 |
...
| `預收餘額` | 列出所有套時數學員已繳費未上課的剩餘金額（單價/剩餘堂數/總價），含合計，超過 15 列自動分頁為 carousel |
| `未打卡查詢` | 選擇月份查詢未補打卡的學員 |
...
| `session_pay` | `{studentId}:{date}` | 單堂學員繳費 → 顯示確認卡片（預設/自訂金額） |
| `confirm_session_pay` | `{studentId}:{date}` | 確認以預設金額繳費 |
| `session_pay_custom` | `{studentId}:{date}` | 進入自訂金額流程（文字輸入） |
...
所有回覆訊息及推播通知皆附帶 Quick Reply 按鈕，方便使用者快速切換功能：
...
coachCheckinForStudent()
    ...
    ├─ 推播通知學員（依收費方式區分）：
    │   ├─ 單堂學員：時段、時長、本堂費用 + 歷史未繳費課程列表
    │   │   └─ 「繳費完成後再麻煩通知教練，謝謝！」
    │   └─ 多堂學員：時段、時長、剩餘時數
    │       ├─ 時數已用完：「目前課程時數已使用完畢，繳費完成後再麻煩通知教練」
    │       └─ 時數不多：「剩餘課程時數不多，繳費完成後再麻煩通知教練」
    ├─ 檢查當前桶是否剛好用完且無下一桶
    │   └─ 是 → 額外推播繳費提醒
    └─ 回覆教練：打卡成功 + 剩餘時數
```
...
│   │   ├── session-payment-confirm.ts # 單堂繳費確認卡片（預設/自訂金額）
